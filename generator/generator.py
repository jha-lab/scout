...
class Generator:
	...
	def return_attention(self,interv_time):
		...
		cont_target, disc_target, attention = self.model.return_attention(pre_int_seq, post_int_seq,\
																timestamp_preint, timestamp_postint,\
																seqid_pre_int,seqid_post_int,\
																target_mask_preint,target_mask_postint,\
																attention_mask_preint,attention_mask_postint,
	...
	def sliding_attention(self):

		self.model.eval()
		all_attentions = []

		for i in range(self.interv_time,self.time_range):
			attention = self.return_attention(i)
